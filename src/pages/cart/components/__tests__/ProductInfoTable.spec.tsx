...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  expect(within(firstItem).getByText('3')).toBeInTheDocument();
  expect(within(firstItem).getByText('2427')).toBeInTheDocument();

  // Assert: 두 번째 아이템의 이름, 수량, 합계 금액을 확인합니다.
  expect(
    within(secondItem).getByText('Awesome Concrete Shirt')
  ).toBeInTheDocument();
  expect(within(secondItem).getByText('4')).toBeInTheDocument();
  expect(within(secondItem).getByText('1768')).toBeInTheDocument();
});
...
it('특정 아이템의 수량이 변경되었을 때 값이 재계산되어 올바르게 업데이트 된다', async () => {
  ...
  const quantityInput = within(firstItem).getByRole('spinbutton');
  await user.clear(quantityInput);
  await user.type(quantityInput, '5');

  // Assert: 수량이 변경된 후 재계산된 금액이 올바르게 표시되는지 확인합니다.
  expect(within(firstItem).getByText('4045')).toBeInTheDocument();
});
...
it('특정 아이템의 수량이 1000개로 변경될 경우 "최대 999개 까지 가능합니다!"라고 경고 문구가 노출된다', async () => {
  ...
  expect(alertSpy).toHaveBeenCalledWith('최대 999개까지 가능합니다!');
});
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  await waitFor(() => {
    expect(
      within(secondItem).queryByText('Awesome Concrete Shirt')
